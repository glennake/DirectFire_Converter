...
def parse(logger, src_config, routing_info=""):
    ...
    logger.log(2, __name__ + ": parse network objects")

    for re_match in re.finditer("object network (.*)\n(?: .*\n){1,}", src_config):

        network_object = str(re_match.group(0)).split("\n")

        if "nat" not in network_object[1]:

            network_object_name = network_object[0][15:]

            data["network_objects"][network_object_name] = {}
            data["network_objects"][network_object_name]["description"] = ""

            for entry in network_object[1:]:

                entry_list = entry.lstrip().split(" ")

                if entry_list[0] == "host":

                    data["network_objects"][network_object_name]["type"] = "host"
                    data["network_objects"][network_object_name]["host"] = entry_list[1]

                elif entry_list[0] == "subnet":
    ...
    for re_match in re.finditer("object-group network (.*)\n(?: .*\n){1,}", src_config):
        ...
        for member in network_groups[i:]:
            ...
            network_object = str(member).lstrip().split(" ")
            ...
            if network_object[0] == "network-object":
                ...
                if network_object[1] == "object":  ## is object member
                    ...
                    if network_object[2] == "255.255.255.255":  ## is a host

                        network_object_name = network_object[1]
                        ...
                        network_object_name = (
                            network_object[1] + "_" + network_object[2]
                        )
                ...
                ## add network group to the group
    ...
    for re_match in re.finditer("object service (.*)\n(?: .*\n){1,}", src_config):

        service_object = str(re_match.group(0)).split("\n")

        service_object_name = service_object[0][15:]

        data["service_objects"][service_object_name] = {}
        data["service_objects"][service_object_name]["description"] = ""

        for entry in service_object[1:]:
                ...
                if acl_rule[i] == "object":  ## source object
                    ...
                    if acl_rule[i + 1] == "255.255.255.255":  ## check if a host
                        ...
                        if network_object not in data["network_objects"]:
                            ...
                            data["network_objects"][network_object]["mask"] = acl_rule[
                                i + 1
                            ]
                            data["network_objects"][network_object]["description"] = ""
